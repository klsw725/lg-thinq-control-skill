...
2. **디바이스 선택** → `--setup` 또는 `--select`로 사용할 기기 지정
3. **상태 조회** → `--state`로 현재 상태 확인
4. **프로파일 조회** → `--profile`로 제어 가능한 속성 확인
5. **제어** → `--control`로 기기 제어
...
> API 호출 전에 반드시 `--check-env`로 환경변수 설정 상태를 먼저 확인한다. 토큰 값은 마스킹되어 출력된다.
